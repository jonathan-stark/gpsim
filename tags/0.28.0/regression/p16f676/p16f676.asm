...
start
	...
	movlw	0xff
	movwf	PORTA
   .assert "(porta & 0x17) == 0, \"**FAILED 16f676  analog bits read 0\""
	nop
	clrf	PORTA
	movwf	PORTA
	movf	PORTA,W
    ...
    .assert "cmcon == 0x1f, \"**FAILED 16f676 CMCON read 0 bits\""
	nop
...
;
  ...
  .assert  "\"*** PASSED 16f676 Functionality\""
	goto	$
...
;
	...
	movlw  0x01
	movwf  CMCON
   .assert "cmcon == 1, \"*** FAILED 16f676 cmcon Comp mode 1 Cinv 0 v+ > v-\""
	nop
   .assert "(porta & 0x04) == 0, \"*** FAILED 16f676 porta2 Comp mode 1 Cinv 0 v+ > v-\""
	nop
	bsf	CMCON,CINV		; Cinv = 1
   .assert "cmcon == 0x51, \"*** FAILED 16f676 cmcon Comp mode 1 Cinv 1 v+ > v-\""
	nop
	bsf	CMCON,CIS		; CIS = 1
   .assert "cmcon == 0x59, \"*** FAILED 16f676 cmcon Comp mode 1 Cinv 1 CIS 1 v+ > v-\""
	nop
   .assert "(porta & 0x04) == 0x04, \"*** FAILED 16f676 porta2 Comp mode 1 Cinv 1 v+ > v-\""
	nop
	movlw  0x01
	movwf  CMCON		; clear CINV and CIS
	movlw  0x10
	movwf  PORTA	; AN0 0 AN1 1
   .assert "cmcon == 0x41, \"*** FAILED 16f676 Comp mode1 v+ < v-\""
	nop
   .assert "(porta & 0x04) == 0x04, \"*** FAILED 16f676 porta2 Comp mode 1 Cinv 1 v+ < v-\""
	nop
	bsf	CMCON,CINV
   .assert "cmcon == 0x11, \"*** FAILED 16f676 cmcon Comp mode 1 Cinv 1 v+ < v-\""
	nop
   .assert "(porta & 0x04) == 0, \"*** FAILED 16f676 porta2 Comp mode 1 Cinv 1 v+ < v-\""
	nop
	nop
;
...
;
	...
	movlw	0x03
	movwf	CMCON
   .assert "cmcon == 0x03, \"*** FAILED 16f676 cmcon Comp mode 3 Cinv 0 AN1 5V\""
	nop
	bcf	PORTA,4		; drive AN1 to low
   .assert "cmcon == 0x43, \"*** FAILED 16f676 cmcon Comp mode 3 Cinv 0 AN1 0V\""
	nop
	bsf	CMCON,CIS		; CIS should not change output
   .assert "cmcon == 0x4b, \"*** FAILED 16f676 cmcon Comp mode 3 Cinv 0 Cis 1 AN1 0V\""
	nop
...
;
   ...
   .assert "cmcon == 0x45, \"*** FAILED 16f676 cmcon Comp mode 5 CIS 0 AN0 0V AN1 0V\""
	nop
	bsf	PORTA,5	; Drive AN0 high (AN1 low)
   .assert "cmcon == 0x45, \"*** FAILED 16f676 cmcon Comp mode 5 CIS 0 AN0 5V AN1 0V\""
	nop
	bsf	CMCON,CIS	; select AN0 (PORTA0)
   .assert "cmcon == 0x0d, \"*** FAILED 16f676 cmcon Comp mode 5 CIS 1 AN0 5V AN1 0V\""
	nop
	movlw	0x10	; Drive AN0 low AN1 High
	movwf	PORTA
   .assert "cmcon == 0x4d, \"*** FAILED 16f676 cmcon Comp mode 5 CIS 1 AN0 0V AN1 5V\""
	nop
	bsf	PORTA,5	; Drive AN0 high (AN1 high)
   .assert "cmcon == 0x0d, \"*** FAILED 16f676 cmcon Comp mode 5 CIS 1 AN0 5V AN1 5V\""
	nop
;
...
test_tmr1:
  ...
  ; TMR1 not running yet, TMR1H and TMR1L should be unchanged
   ...
   .assert "W==0, \"*** FAILED 16f676 TMR1 test TMR1L read\""
        nop
        MOVF    TMR1H,W
   .assert "W==0x80, \"*** FAILED 16f676 TMR1 test TMR1H read\""
        nop
...
tmr1_loop2:
   ...
   .assert "tmr1l == W, \"*** FAILED 16f676 TMR1 test - TMR1 stop\""
	nop
	return
...
tmr1_loop3:
   ...
   .assert "tmr1l != W, \"*** FAILED 16f676 TMR1 test - TMR1 running\""
	nop
	return
...
eefail:
  .assert "\"***FAILED 16f676 eerom write/read error\""
	nop
...
test_adc:
  ...
  ; ANS = 1 AN0 is only analog port
	...
	bsf	ADCON0,ADON	; enable ADC
        ...
        call    Convert
   .assert "W == 0x66, \"*** FAILED 16f676 ADC test - AN0 = 2 Vref = Vddi ADFM=0\""
	nop
	...
	bsf	ADCON0,VCFG	; AN1 as hi Vref 
        call    Convert
   .assert "W == 0x80, \"*** FAILED 16f676 ADC test - AN0 = 2 Vref = AN1 = 4 ADFM=0\""
	nop
	bsf	ADCON0,ADFM	; AN1 as hi Vref , ADFM=1
        call    Convert
   .assert "W == 0x02, \"*** FAILED 16f676 ADC test - AN0 = 2 Vref = AN1 = 4 ADFM=0\""
	nop
