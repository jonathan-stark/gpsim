...
.command macro x
  ...
  .sim "p16f677.BreakOnReset = false"
  .sim "break c 0x10000"
  .sim "p16f677.frequency=10000"
...
;	when the WDT goes off sleeps continue
	sleep
	nop
  .assert "(status & 0x18) == 0x00,\"*** FAILED 16f677 status after sleep\""
	nop
...
;	Test the WDT cause a reset in under 2 * delay1
	clrwdt
  .assert "(status & 0x18) == 0x18,\"*** FAILED 16f677 status after clrwdt\""
	nop
	call	delay1
	call	delay1
...
FAILED:
  .assert "\"*** FAILED p16f677 no WDT reset\""
	goto	$

FAILED2:
  .assert "\"*** FAILED p16f677 unexpected WDT reset\""
	goto	$
...
wdt_reset:
  ...
  .assert "(status & 0x18) == 0x08,\"*** FAILED 16f677 status after WDT Reset\""
    nop
    .assert "\"*** PASSED p16f677 WDT\""
    goto $
  end
