        ...
        ;; TMR1 for a p18f2620 pic (like the 18fxxx family) although
	...
	list    p=18f2620                ; list directive to define processor
...
Start:	
  ...
  ; TMR1 not running yet, TMR1H and TMR1L should be unchanged
   ...
   .assert "W==0, \"*** FAILED p18f2620 TMR1 test TMR1L read\""
	nop
	MOVF	TMR1H,W
   .assert "W==0x80, \"*** FAILED p18f2620 TMR1 test TMR1H read\""
	nop
...
L1:	clrwdt
   ...
   .assert "(SecondsLo==6), \"*** FAILED p18f2620 TMR1 test bad count with crystal\""
	nop
...
L2:	clrwdt
   ...
   .assert "(SecondsLo==64), \"*** FAILED p18f2620 TMR1 test bad count with crystal\""
	nop
...
L3:	clrwdt
   ...
   .assert "SecondsLo == 13, \"*** FAILED p18f2620 TMR1 test bad count with stimuli\""
	nop
done:
  .assert  "\"*** PASSED p18f2620 TMR1 test\""
        bra     $

failed:
        movlw   1
        movwf   failures
  .assert  "\"*** FAILED p18f2620 TMR1 test\""
        bra     done
