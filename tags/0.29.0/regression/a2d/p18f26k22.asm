...
check_TMR0_interrupt:
  ...
  .assert "\"FAIL 18F26k22 unexpected interrupt\""
	nop
	RETFIE 1		; Then leave
...
MAIN    CODE
    ...
    .sim "p18f26k22.xpos = 192"
    .sim "p18f26k22.ypos = 48"
...
Start:
  ...
  .assert "adresh == 0xff, \"FALIED 18F26k22 a2d AN0=5V\""
	nop
...
;  .command "V1.resistance=1e6"
	...
	movlw	0xff
        movwf   ANSELA          ;Configure porta to be completely analog
	bsf	ADCON0,CHS0	;Use AN1
                                                                                

	RCALL	Convert

  .assert "adresh == 0x00, \"FAILED 18F26k22 Digital low\""
	nop
  ...
  .assert "adresh == 0xff, \"FAILED 18F26k22 Digital high\""
	nop
  ...
  .command "V1.voltage=1.0"
   ...
   .assert "adresh == 0x33, \"FAILED 18F26k22 AN0=1V\""
        nop
  ...
  .command "V2.voltage=2.0"
   ...
   .assert "adresh == 0x80, \"FAILED 18F26k22 AN0=1V Vref+=2V\""
        nop
	...
	goto	$-2
	movlw	0x7d
	movwf	ADCON0		; read Vref 1.024
	rcall	Convert
	nop
   .assert "adresh == 0x83, \"FAILED 18F26k22 FVR=1.024V Vref+=2V\""
        nop

done:
  .assert  "\"*** PASSED 18F26k22 a2d test\""
        bra     $
