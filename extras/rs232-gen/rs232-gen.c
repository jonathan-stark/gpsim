...
static int output_file(void) {
  ...
  dprintf(fd, "# rs232 Stimulus generated by rs232-gen\n\n");
  dprintf(fd, "stimulus asynchronous_stimulus\n");
  dprintf(fd, "initial_state 1\n");
  dprintf(fd, "start_cycle 0\n\n");
  ...
  while ((data[i].size != -1) && (i < MAX_DATA)) {
    ...
    for (j = 0; j < data[i].size; j++) {
      dprintf(fd, "# Character: 0x%02X", data[i].data[j]);
