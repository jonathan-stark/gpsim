...
;	when the WDT goes off sleeps continue
	sleep
	nop
  .assert "(status & 0x18) == 0x00,\"*** FAILED 16f628 status after sleep\""
	nop
...
;	Test the WDT cause a reset in under 2 * delay1
	clrwdt
  .assert "(status & 0x18) == 0x18,\"*** FAILED 16f628 status after clrwdt\""
	nop
	call	delay1
	call	delay1
...
FAILED:
  .assert "\"*** FAILED p16f628 no WDT reset\""
	goto	$

FAILED2:
  .assert "\"*** FAILED p16f628 unexpected WDT reset\""
	goto	$
...
wdt_reset:
  ...
  .assert "(status & 0x18) == 0x08,\"*** FAILED 16f628 status after WDT Reset\""
    nop
    .assert "\"*** PASSED p16f628 WDT\""
    goto $
  end
