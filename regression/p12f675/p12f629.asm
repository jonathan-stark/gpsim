...
test_tmr1:
  ...
  ; TMR1 not running yet, TMR1H and TMR1L should be unchanged
   ...
   .assert "W==0, \"*** FAILED 12f629 TMR1 test TMR1L read\""
        nop
        MOVF    TMR1H,W
   .assert "W==0x80, \"*** FAILED 12f629 TMR1 test TMR1H read\""
        nop
...
;
        sleep
	nop
   .assert "(status & 0x18) == 0, \"*** FAILED 12f629 TMR1 test - WDT wake\""
        nop
        BANKSEL PIE1
...
tmr1_loop2:
   ...
   .assert "tmr1l == W, \"*** FAILED 12f629 TMR1 test - TMR1 stop\""
	nop
	return
...
tmr1_loop3:
   ...
   .assert "tmr1l != W, \"*** FAILED 12f629 TMR1 test - TMR1 running\""
	nop
	return
...
;
	...
	movlw  0x01
	movwf  CMCON
   .assert "cmcon == 1, \"*** FAILED 12f629 cmcon Comp mode 1 Cinv 0 v+ > v-\""
	nop
   .assert "(gpio & 0x04) == 0, \"*** FAILED 12f629 gpio2 Comp mode 1 Cinv 0 v+ > v-\""
	nop
	bsf	CMCON,CINV		; Cinv = 1
   .assert "cmcon == 0x51, \"*** FAILED 12f629 cmcon Comp mode 1 Cinv 1 v+ > v-\""
	nop
	bsf	CMCON,CIS		; CIS = 1
   .assert "cmcon == 0x59, \"*** FAILED 12f629 cmcon Comp mode 1 Cinv 1 CIS 1 v+ > v-\""
	nop
   .assert "(gpio & 0x04) == 0x04, \"*** FAILED 12f629 gpio2 Comp mode 1 Cinv 1 v+ > v-\""
	nop
	movlw  0x01
	movwf  CMCON		; clear CINV and CIS
	movlw  0x10
	movwf  GPIO	; AN0 0 AN1 1
   .assert "cmcon == 0x41, \"*** FAILED 12f629 Comp mode1 v+ < v-\""
	nop
   .assert "(gpio & 0x04) == 0x04, \"*** FAILED 12f629 gpio2 Comp mode 1 Cinv 1 v+ < v-\""
	nop
	bsf	CMCON,CINV
   .assert "cmcon == 0x11, \"*** FAILED 12f629 cmcon Comp mode 1 Cinv 1 v+ < v-\""
	nop
   .assert "(gpio & 0x04) == 0, \"*** FAILED 12f629 gpio2 Comp mode 1 Cinv 1 v+ < v-\""
	nop
	nop
;
...
;
	...
	movlw	0x03
	movwf	CMCON
   .assert "cmcon == 0x03, \"*** FAILED 12f629 cmcon Comp mode 3 Cinv 0 AN1 5V\""
	nop
	bcf	GPIO,4		; drive AN1 to low
   .assert "cmcon == 0x43, \"*** FAILED 12f629 cmcon Comp mode 3 Cinv 0 AN1 0V\""
	nop
	bsf	CMCON,CIS		; CIS should not change output
   .assert "cmcon == 0x4b, \"*** FAILED 12f629 cmcon Comp mode 3 Cinv 0 Cis 1 AN1 0V\""
	nop
...
;
   ...
   .assert "cmcon == 0x45, \"*** FAILED 12f629 cmcon Comp mode 5 CIS 0 AN0 0V AN1 0V\""
	nop
	bsf	GPIO,5	; Drive AN0 high (AN1 low)
   .assert "cmcon == 0x45, \"*** FAILED 12f629 cmcon Comp mode 5 CIS 0 AN0 5V AN1 0V\""
	nop
	bsf	CMCON,CIS	; select AN0 (GPIO0)
   .assert "cmcon == 0x0d, \"*** FAILED 12f629 cmcon Comp mode 5 CIS 1 AN0 5V AN1 0V\""
	nop
	movlw	0x10	; Drive AN0 low AN1 High
	movwf	GPIO
   .assert "cmcon == 0x4d, \"*** FAILED 12f629 cmcon Comp mode 5 CIS 1 AN0 0V AN1 5V\""
	nop
	bsf	GPIO,5	; Drive AN0 high (AN1 high)
   .assert "cmcon == 0x0d, \"*** FAILED 12f629 cmcon Comp mode 5 CIS 1 AN0 5V AN1 5V\""
	nop
;
