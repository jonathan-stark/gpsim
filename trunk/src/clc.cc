...
void CLC::osc_out (bool level, int kind)
{
    ...
    if (gate_change)
      {
	  Dprintf (("CLC%d osc_out() kind=%d level=%d enable=%d\n", index + 1,
...
void CLC::CxOUT_sync (bool level, int cm)
{
    if (CMxOUT_level[cm] != level)
	  ...
	  if (gate_change)
	    {
		Dprintf (("CLC%d C%dOUT_sync() level=%d enable=%d\n", index + 1,
...
{
    Dprintf (("CLC%d out_pwm() pwm%d level=%d enable=%d\n", index + 1, id + 1,
	      level, CLCenabled ()));
...
void CLC::setState (char new3State, int id)
{
    ...
    if (state != INxstate[id])
	  ...
	  if (gate_change)
	    {
		Dprintf (("CLC%d setState() IN%d level=%d enable=%d\n",
...
void CLC::outputCLC (bool out)
{
    ...
    Dprintf (("outputCLC CLC%d out=%d old_out=%d clcdata=0x%x\n", index, out,
    ...
    assert (m_Interrupt);
    Dprintf (("CLC::outputCLC CLC%d old_out %d out %d int 0x%x \n", index + 1,
...
void CLC::lcxupdate (bool bit_val, unsigned int pos)
{
    bool update = false;
    for (int i = 0; i < 4; i++)
      {
    ...
    if (update)
      {
	  if (CLCenabled ())
	      Dprintf (("CLC%d lcxupdate LC%d_OUT=%d\n", index + 1, pos + 1,
			bit_val));
	  compute_gates ();
      }
}
...
void CLC::config_inputs (bool on)
{
    ...
    Dprintf (("config_inputs CLC%d on=%d active_gates=0x%x\n", index + 1, on,
	      active_gates));
    ...
    if (haveIN0 && (INxactive[0] ^ on))
      {
	  Dprintf (("config_inputs CLC%d IN0 on=%d\n", index + 1, on));
	  enableINxpin (0, on);
      }
    else if (!haveIN0 && INxactive[0])
      {
	  Dprintf (("config_inputs CLC%d IN0 OFF on=%d\n", index + 1, on));
	  enableINxpin (0, false);
      }
    if (haveIN1 && (INxactive[1] ^ on))
      {
	  Dprintf (("config_inputs CLC%d IN1 on=%d\n", index + 1, on));
	  enableINxpin (1, on);
      }
    else if (!haveIN1 && INxactive[1])
      {
	  Dprintf (("config_inputs CLC%d IN1 OFF on=%d\n", index + 1, on));
	  enableINxpin (0, false);
      }

    if (haveFRC && (FRCactive ^ on))
      {
	  Dprintf (("config_inputs CLC%d FRC FRCactive=%d on=%d\n", index + 1,
		    FRCactive, on));
	  FRCactive = on;
      ...
      {
	  Dprintf (("config_inputs CLC%d FRC OFF  on=%d\n", index + 1, on));
	  FRCactive = false;
	  frc->start_osc_sim (false);
      }
    if (haveLFINTOSC && (LFINTOSCactive ^ on))
      {
	  Dprintf (("config_inputs CLC%d LFINTOSC LFINTOSCactive=%d on=%d\n",
      ...
      {
	  Dprintf (("config_inputs CLC%d LFINTOSC OFF  on=%d\n", index + 1,
		    on));
    ...
    if (haveHFINTOSC && (HFINTOSCactive ^ on))
      {
	  Dprintf (("config_inputs CLC%d HFINTOSC HFINTOSCactive=%d on=%d\n",
      ...
      {
	  Dprintf (("config_inputs CLC%d HFINTOSC OFF  on=%d\n", index + 1,
		    on));
	  HFINTOSCactive = false;
...
// Inputs for p10f32x
CLC1::CLC1 (Processor * cpu, unsigned int _index, CLCDATA * _clcdata):
