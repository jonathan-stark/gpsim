...
start:
   ...
   .sim "attach na4 V4.pin porta2"
	...
	call	Convert
   .assert "adresh == 0xff, \"FAILED 16F917 inital test\""
	nop
	bsf	ADCON0,ADFM
	call	Convert
   .assert "adresh == 0x03, \"FAILED 16F917 ADFM=1 Vin==Vdd chan=1 test\""
	nop
	...
	movwf	ADCON1		
  .assert "adcon1 == 0x70, \"FAILED 16F917 invalid ADCON1 bits writable\""
	nop
;RRR	clrf	ANSEL		;Configure porta to be completely digital
  ...
  .assert "adresh == 0x00, \"FAILED 16F917 Digital low\""
	nop
  ...
  .assert "adresh == 0xff, \"FAILED 16F917 Digital high\""
	nop
	;;
  ...
  .assert "adresh == 0xff, \"FAILED 16F917 AN1=5V\""
	nop
  ...
  .command "V1.voltage=1.0"
   ...
   .assert "adresh == 0x33, \"FAILED 16F917 AN1=1V\""
	nop
  ...
  .command "V2.voltage=2.0"
   ...
   .assert "adresh == 0x80, \"FAILED 16F917 AN1=1V Vref+=2V\""
	nop
   ...
   .assert "adresh == 0x02, \"FAILED 16F917 ADFM=1 AN1=1V Vref+=2V\""
	nop

	bsf	ADCON0,VCFG1
	bcf	ADCON0,ADFM
	call	Convert
   .assert "adresh == 0x55, \"FAILED 16F917 ADFM=0 AN1=1V Vref+=2V Vref-=0.5V\""
	nop


  .assert  "\"*** PASSED p16f917 a2d, eeprom test\""
